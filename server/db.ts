...
export async function listTables(schema='public') {
  const res: QueryResult<QueryResultRow> = await pool.query(`select * from information_schema.tables where table_schema = '${schema}' and table_type = 'BASE TABLE'`)
  // console.log("### tables: ", res.rows);
  // console.log("### tables count: ", res.rowCount);
  return res;
}

export async function listColumns(schema='public', table) {
  const res: QueryResult<QueryResultRow> = await pool.query(`select * from information_schema.columns where table_schema = '${schema}' and table_name = '${table}'`)
  // console.log("### tables: ", res.rows);
  // console.log("### tables count: ", res.rowCount);
  return res;
}

export async function listContraints(schema='public') {
  const res: QueryResult<QueryResultRow> = await pool.query(`
